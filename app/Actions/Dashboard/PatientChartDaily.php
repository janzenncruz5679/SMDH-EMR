...
class PatientChartDaily
{
    public static function getDataForChartsDaily()
    {
        ...
        $labels_daily = $groupedRecord->keys()->toArray();
        $data_daily = $groupedRecord->map(function ($item) {
            return $item['count'];
        })->toArray();

        // dd($labels, $data);
        return [$labels_daily, $data_daily];
    }
}