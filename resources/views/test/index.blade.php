		...
		<div class="admissionDisplay h-full w-full grid gap-4">
			...
			<div class="searchBar h-12 w-full flex justify-start items-center">
				...
				<div class="addpatientBar h-full w-full flex items-center justify-end">
					<button class="btnAddpatient h-full w-48 text-[1.5rem] bg-blue-300 tracking-[2px] text-white rounded-[15px] transform transition hover:-translate-y-0.5 hover:bg-blue-100"><a href="{{ url('/patientPage/addPatient') }}">
						...
						@foreach ($patientDatas as $patientData)
							<tr class="grid grid-cols-12 even:bg-gray-200 odd:bg-white text-xl">
								<td class="flex justify-center">
									{{ $patientData->patient_id }}</td>
								<td class="col-span-5 flex justify-center">
									{{ $patientData->patient->fname }}
								</td>
								<td class="flex justify-center">
									{{ $patientData->patient->bdate->age }}
								</td>
								<td class="flex justify-center">
									{{ $patientData->patient->sex }}
								</td>
								<td class="col-span-2 flex justify-center">
									{{ $patientData->patient->contact_num }}
								</td>
								<td class="col-span-2 flex justify-center">
									<div class="grid grid-cols-3 justify-center gap-4">
										<a href="{{ route('test-patient.show', $patientData->id) }}" class="editIcon hover:text-blue-300">
											<i class="fa-solid fa-eye"></i>
										</a>
										<a href="{{ url('/patientPage/updateAdmission' . $patientData->id) }}" class="editIcon hover:text-blue-300">
											<i class="fa-solid fa-edit"></i>
										</a>
										<a href="{{ url('/patientPage/savepdfAdmission' . $patientData->id) }}" class="editIcon hover:text-blue-300">
