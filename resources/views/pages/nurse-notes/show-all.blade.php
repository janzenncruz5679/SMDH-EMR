					...
					<div class="form-section">
						<div class="grid grid-cols-8 border-b-2 border-black h-full">
							...
							<div class="col-span-2 border-r-2 border-black p-3">
								...
								<input type="text" readonly
									class="w-full h-10 border-4 border-blue-300 focus:border-blue-200 focus:outline-blue-200 px-[10px] focus:outline-offset-2"
									placeholder="Ward #" name="ward" autocomplete="off" value="{{ $nurseNotes->first()->ward_room }}">
								{{-- <span class="text-base font-[sans-serif] font-medium text-red-600">
                                     @error('patient_fullname')
                                         {{ $message }}
                                     @enderror
						...
						@forelse ($nurseNotes as $note)
							<div class="p-3 grid gap-3">
								<label class=" text-xl text-[#003D33] font-semibold tracking-wider">{{ $note->date_time->format('F d, Y - h:i A') }}</label>
									...
									<div class="grid gap-3">
										<div class="grid">
											...
											<input type="date" readonly
												class="w-full h-10 border-4 border-blue-300 focus:border-blue-200 focus:outline-blue-200 px-[10px] focus:outline-offset-2"
												name="obsDate[]" autocomplete="off" value="{{ $note->date_time->toDateString() ?? '' }}">
										</div>
										{{-- <span class="text-base font-[sans-serif] font-medium text-red-600">
                                         @error('patient_fullname')
                                             {{ $message }}
										...
										<div class="grid">
											...
											<input type="time" readonly
												class="w-full h-10 border-4 border-blue-300 focus:border-blue-200 focus:outline-blue-200 px-[10px] focus:outline-offset-2"
												name="obsTime[]" autocomplete="off" value="{{ $note->date_time->toTImeString() ?? '' }}">
										</div>
									</div>
