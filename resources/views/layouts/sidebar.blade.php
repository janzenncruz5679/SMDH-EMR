<div class="sidebar z-50 fixed w-[275px] h-screen left-0 top-[59px] bg-zinc-900">
	<div class="user pt-[30px] pb-[10px] px-0 h-[25vh] flex flex-col justify-center">
		...
		<div class="user-name p-0 text-center">
			<a href="{{ url('home') }}" class="p-0 text-[2.5rem] text-white hover:text-white tracking-[3px]">{{ Auth::user()->name }}</a>
		</div>
	</div>
		...
		<nav>
			<ul>
				<li class="flex items-center justify-start cursor-pointer text-white"><a
						class="{{ Str::contains(URL::current(), url('home')) ? __('bg-blue-300') : null }} selection_menu hover:text-black hover:bg-blue-100 w-[275px] p-[20px] text-[1.5rem] no-underline tracking-[3px]" href="{{ url('home') }}"><i
				...
				<li class="flex items-center justify-start cursor-pointer text-white"><a
						...
						href="{{ url('patientPage') }}"><i class="fa-solid fa-hospital-user  px-[15px] py-0 text-[1.5rem]"></i>{{ __('Patients') }}</a>
				</li>
				<li class="flex items-center justify-start cursor-pointer text-white"><a
						class="{{ Str::contains(URL::current(), url('stations')) ? __('bg-blue-300') : null }} selection_menu hover:text-black hover:bg-blue-100 w-[275px] p-[20px] text-[1.5rem] no-underline tracking-[3px]" href="{{ url('stations') }}"><i
				...
				<li class="flex items-center justify-start cursor-pointer text-white"><a
						...
						href="{{ route('billingTable') }}"><i class="fa-solid fa-hand-holding-dollar px-[15px] py-0 text-[1.5rem]"></i>{{ __('Billing') }}</a>
				</li>
			</ul>
		</nav>
	</div>
	...
	<div class="logout_button p-[10px] relative top-[120px]">
		...
		@else
			<button
				...
				<p class="relative w-full h-full flex items-center justify-center hover:text-black transition delay-150 duration-300 ease-in-out">
					{{ __('Logout') }}</p>
			</button>
